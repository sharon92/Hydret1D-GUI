...
class layerPropPopup(QDialog):
    def __init__(self,app,itemT,rauEditor=False,parent=None):
        super().__init__(parent)
        uic.loadUi(ui,self)
        self.apply.clicked.connect(self.accept)
        self.cancel.clicked.connect(self.reject)
        self.pitems = app.lw_dictL[itemT]
        if rauEditor:
            self.pitem = self.pitems[2]
        elif type(self.pitems) == list:
...
def changePropsL(self,itemT):
    P = layerPropPopup(self,itemT,rauEditor = True)
    if P.exec_():
